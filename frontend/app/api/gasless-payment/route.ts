...
import { PUSH_TESTNET, QUIKPAY_CONTRACT_ADDRESS, QUIKPAY_ABI } from '@/lib/contract'
...
export async function POST(request: Request) {
  try {
    ...
    if (Number(auth.chainId) !== PUSH_TESTNET.id) {
    ...
    const rpcUrl =
      process.env.RPC_URL ||
      process.env.NEXT_PUBLIC_PUSH_TESTNET_RPC_URL ||
