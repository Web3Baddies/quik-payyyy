...
async function main() {
  ...
  if (!signers || signers.length === 0) {
    throw new Error(
      ...
      "Also confirm networks.pushTestnet.accounts is configured in hardhat.config.js."
    );
  }
  ...
  try {
    ...
    const outPath = path.join(outDir, `${network.name || 'pushTestnet'}-latest.json`);
  ...
  console.log("\nNext:");
  console.log(`  npx hardhat verify --network ${network.name || 'pushTestnet'} ${contractAddress}`);
  
  console.log("\n📋 Deployment Summary:");
  console.log(JSON.stringify(deploymentInfo, null, 2));
  
  // Verify contract (optional)
  if (network.chainId === 42101n || network.chainId === 421011n) {
...
function getExplorerUrl(chainId, address) {
  switch (chainId.toString()) {
    case "42101":
      return `https://donut.push.network/address/${address}`;
    case "421011":
      return `https://explorer.push.network/address/${address}`; // Placeholder for mainnet
    default:
